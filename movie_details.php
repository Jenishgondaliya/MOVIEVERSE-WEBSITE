...
if(isset($_GET['id'])){
    ...
    // $querry = $conn->query("SELECT trailer_link FROM `movie_list` where id = '{$_GET['id']}'");
    // $convert = str_replace("watch?v=","embed/",$querry);
    // $tname = $conn->query("SELECT title FROM `movie_list` where id = '{$_GET['id']}'");
    // echo '$querry';
            ...
            <div class="container-fluid">
                        ...
                        <div class="row">
                            <div class="col-12">
                                <table class="table table-bordered border-info">
                                    ...
                                    <tr>
                                        <th class="text-muted text-white bg-gradient-info px-2 py-1">Predicted Success
                                            Rate:</th>
                                        <td>
                                            <?php if($count_reviews  > 0): ?>
                                            <b><?= $success_rate ?>/5</b> <span class="text-muted">or</span>
                                            <b><?= $success_perc."%" ?></b>
                                            <?php else: ?>
                ...
                <?php
                    ...
                    $con= new mysqli($DB_SERVER,$DB_USERNAME,$DB_PASSWORD,$DB_NAME);
                        ...
                        $qryy = "SELECT * FROM `movie_list` where id = '{$_GET['id']}'";
                        $result= $con->query($qryy);
                        $data = $result->fetch_assoc();
                        $video = $data['trailer_link'];
                        $titl = $data['title'];  
                        echo" <h4>Trailer of ".$titl."  </h4>" ; 
                        
                        echo"<iframe width='560' height='315' src='".$video."' title='YouTube video player' frameborder='0' allow='accelerometer; autoplay=1; clipboard-write; encrypted-media; gyroscope; picture-in-picture ' allowfullscreen ></iframe>";
                ...
                <div class="rounded-0 mt-3">
                            ...
                            <div class="col-xs-12 col-sm-12 col-md-8 col-lx-6">
                                <div class="card card-outline card-primary shadow rounded-0">
                                    <div class="card-header py-1 rounded-0">
                                        <div class="row">
                                            <div class="col-1">
                                                <img src="<?= validate_image($row['avatar']) ?>" alt="User Avatar"
                                                    class="review-user-img">
                                            </div>
                                            <div class="col-9" style="line-height: .5em;">
                                                <h4 class="card-title w-100"><b><?= $row['title'] ?></b></h4>
                                                <small><span class="text-primary"><?= $row['email'] ?></span> <span
                                                        class="mx-3"><?= date("Y-m-d H:i", strtotime($date_created)) ?></span></small>
                                            </div>
                                            <div class="col-2">
                                    ...
                                    <div class="card-body rounded-0">
                                        ...
                                        <div class="row">
                                            <div class="form-group col-md-12 text-right">
                                                <button class="btn btn-danger btn-sm btn-flat delete_data" type="button"
                                ...
                                <form action="" id="comment-form">
                                    ...
                                    <div class="row">
                                        <div class="col-md-6 form-group">
                                            ...
                                            <input type="text" class="form-control form-control-sm rounded-0" id="title"
                                                name="title" required>
                                        </div>
                                    </div>
                                    <div class="row">
                    ...
                    <!-- End of Comment Form -->
                    <?php else: ?>
                    <center><small class="text-muted"><i>You have submitted your review for this movie.</i></small>
                    </center>
                    <?php endif; ?>
...
$(function() {
    $('.delete_data').click(function() {
        _conf("Are you sure to delete your review on this movie?", "delete_review", [$(this).attr(
            'data-id')])
    })
